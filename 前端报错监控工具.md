...
26  const handleError = ( err, vm, info ) =>{
27    const host = 'http://8.134.138.188'
28    const path = options ? `/${options}/` : '/'
29    if( vm ) {  // 非axios请求错误
30      // console.log(`Error: ${err.toString()}\nInfo: ${info}\nvm: ${vm.constructor.extendOptions.name}`)
31      (new Image()).src = `${host}${path}log?p=${location.href}&e=${err.toString()}&i=${info.toString()}&v=${vm.constructor.extendOptions.name}`
32  
33    } else {  // axios请求错误
34 
35      (new Image()).src = `${host}${path}log?p=${location.href}&e=${err.toString()}`
36  
37    }
38  }
39
40  Vue.config.errorHandler = handleError
41  Vue.prototype.$throw = ( error, vm = null, info = null ) => handleError( error, vm,  info)
42}
```
